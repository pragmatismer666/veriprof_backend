...
class Support_model extends CI_Model
{
    ...
    {   
        ...
        for ($i=0;$i<sizeof($data);$i++){
            $query = $this->db->select("*")->from('tbl_staffs')->where(array('buzi_id'=>$buzi_id,'created_by'=>$user_id, 'email'=>$data[$i]['email'], 'phone'=>$data[$i]['phone']))->get();
            if (sizeof($query->result()) > 0){ $staffNum++; continue; }            
        ...
        if( sizeof($data) > $staffNum ) { return false; }
        return true;
    }
    ...
    {    
        ...
        for ($i=0; $i<sizeof($equips); $i++){
            ...
            if (sizeof($query->result()) > 0){ $equipNum++; continue; }
            $each = array();
        ...
        if (sizeof($equips)>$equipNum){ return false; }
        return true;
    }
    ...
    {   
        ...
        for ($i=0;$i<sizeof($branchs);$i++){
            ...
            if ( sizeof(($query->result())) > 0 ){ $branchsNum++; continue; }
            else {                   
    ...
    function getStaff($data){
        ...
        if (empty($staff) ){ return false; }
            ...
            if (empty($office) ){ return false; }
            else { return $office[0]->email; }
        }
    }
